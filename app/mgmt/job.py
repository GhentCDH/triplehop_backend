...
class JobManager:
    ...
    async def create(self, type: str, project_name: str = None, entity_type_name: str = None) -> uuid.UUID:
        project_id = None
        entity_type_id = None
        if project_name is not None:
            project_id = await self._config_manager.get_project_id_by_name(project_name)
        if entity_type_name is not None:
            entity_type_id = await self._config_manager.get_entity_type_id_by_name(project_name, entity_type_name)
    ...
    async def es_index(self, job_id: uuid.UUID, project_name: str, entity_type_name: str):
        ...
        try:
            entity_types_config = await self._config_manager.get_entity_types_config(project_name)
            ...
            while True:
                ...
                await self._job_repo.update_counter(job_id, (batch_counter + 1) * BATCH_SIZE)
                batch_counter += 1
