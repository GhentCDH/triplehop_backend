...
async def refresh(Authorize: AuthJWT = Depends()):
    ...
    untill_expiration = (
        datetime.datetime.fromtimestamp(Authorize.get_raw_jwt()["exp"])
        - datetime.datetime.now()
    )
    if untill_expiration < 10 * Authorize._access_token_expires:
        tokens["refresh_token"] = Authorize.create_refresh_token(
            subject=Authorize.get_jwt_subject()
        )
...
async def user(
    user_with_permissions: UserWithPermissions = Depends(
